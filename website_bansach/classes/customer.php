...
class customer
{
	...
	}  //phí dịch vụ 3tr nghe
	public function  insert_binhluan($data,$id){
		
		
		$tenbinhluan = mysqli_real_escape_string($this->db->link, $data['tennguoibinhluan']);
		$binhluan = mysqli_real_escape_string($this->db->link, $data['binhluan']);
if($tenbinhluan=='' || $binhluan==''){
$alert = "<span class='error'>Các trường không được để trống </span>";
	     return $alert;
	 }else{
$query = "INSERT INTO tbl_binhluan(tenbinhluan,binhluan,product_id) VALUES('$tenbinhluan','$binhluan','$id')";
...
$alert = "<span class='error'>Email hoặc mật khẩu không trùng khớp </span>";
	     return $alert;
	 }
	 }
	}
...
$query = "UPDATE tbl_customer SET name='$name',zipcode='$zipcode',email='$email',address='$address',phone='$phone' WHERE id='$id'";
	$result = $this->db->insert($query);
	if ($result ){
		$alert = " <span class='success'> Khách hàng đã cập nhập thành công</span> ";
		return $alert;
	}else{
		$alert = " <span class='error'> Khách hàng đã cập nhập không thành công</span> ";
		return $alert;
	}
